...
```
GET    /management/jobs                      → JobRecord[]
GET    /management/jobs/:queue/:jobId        → JobRecord | null
DELETE /management/jobs/:queue/:jobId        → { deleted: boolean }
POST   /management/jobs/:queue/:jobId/pause  → JobRecord | null
POST   /management/jobs/:queue/:jobId/resume → JobRecord | null
POST   /management/jobs/:queue/:jobId/promote→ JobRecord | null
POST   /management/jobs/:queue/:jobId/retry  → JobRecord | null

GET    /management/queues                    → QueueRecord[]
GET    /management/queues/:queue/stats       → QueueStats
POST   /management/queues/:queue/pause       → { ok: true }
POST   /management/queues/:queue/resume      → { ok: true }
POST   /management/queues/:queue/reset       → { ok: true }
```

Query params on `GET /management/jobs`: `?queue=payments&status=failed`
...
### Benchmark (v0.50.0, 100 jobs, concurrency 10, MacBook Pro 16" M1 Pro, Redis via Docker)

| Metric          | InMemoryStorage | Redis (local) |
| --------------- | --------------- | ------------- |
| Duration        | 0.02s           | 0.20s         |
| Throughput      | 4,144 jobs/sec  | 511 jobs/sec  |
| Latency p50     | 2.70ms          | 18.18ms       |
| Latency p95     | 3.52ms          | 29.77ms       |
| Latency p99     | 3.61ms          | 29.94ms       |
| Latency avg     | 2.39ms          | 19.23ms       |
| Latency min     | 1.37ms          | 15.04ms       |
| Latency max     | 3.61ms          | 29.94ms       |
